...
module.exports =
    class AccountsController extends require('./Controller') {
        ...
        remove(id) { // warning! this is not an API endpoint
            ...
            this.deleteAllImagesFromUser(id)
            super.remove(id);
        }
        deleteAllImagesFromUser(userId) {
            let imagesRepository = new ImagesRepository(this.req, true);
            let allImages = imagesRepository.getAll();
            ...
            for (let img of allImages) {
                if (img.UserId == userId)
                    imgToDelete.push(index);
                index++;
            }
            imagesRepository.removeByIndex(imgToDelete);
